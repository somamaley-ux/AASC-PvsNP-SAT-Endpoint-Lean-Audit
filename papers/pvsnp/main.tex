...
On the unrestricted finite CNF-SAT endpoint-image carrier, after the standard CNF-SAT projection is accepted for official resolution, no stable middle endpoint role remains between positive occupation, lawful coequal occupation, and independent non-positive status work. More precisely, suppose the following are fixed: the context-bound CNF-SAT carrier, the official CNF-SAT resolution route, endpoint use of the act, candidate-image non-occupation as its projected endpoint-role content, and same-domain preservation. If the act is not support, not bookkeeping, not repair, not carrier shift, not positive candidate occupation, and not lawful coequal image occupation, then the act is independent same-domain endpoint-status work. In the SAT image this is the separator-discriminator role:
\[
\begin{aligned}
&\ClayCtx\wedge\FixedDom(R)\wedge\ModelOfCnf(model,R)
  \wedge\OfficialCNFSATRoute(R,model)\\
&\quad\wedge\EndpointUse_R(B)\wedge\CandImageExcl(model)\\
&\quad\wedge\neg\SATEndpointImageOcc(model)
  \wedge\neg\LawfulCoequalImageOcc_R(model)\\
&\qquad\Rightarrow \SepDisc(model),
\end{aligned}
\]
provided the act is used as theorem-bearing endpoint resolution rather than support or bookkeeping.
\end{theorem}

\begin{proof}
The hypotheses place the act on the fixed unrestricted finite CNF-SAT candidate-image carrier and make the CNF-SAT correspondence the route of official endpoint resolution. Theorem~\ref{thm:sat-projection-fixes-endpoint-role-content} gives the endpoint-role content of a negative resolution on that route: candidate-image non-occupation. Theorem~\ref{thm:single-occupation-interface-sat} gives the single occupation interface of the image, so positive occupation is exactly encoded non-failing polynomial-time candidate occupation. Corollary~\ref{cor:candidate-image-exclusion-not-coequal-occupation} and Theorem~\ref{thm:negative-coequal-requires-interface-change} exclude the reading of candidate-image exclusion as either positive occupation or coequal image occupation. Endpoint use excludes support-only and bookkeeping roles; fixed domain excludes carrier shift; act-time finality excludes repair. Thus the act is theorem-bearing, endpoint-resolving, same-domain, non-positive, non-coequal, non-support, non-bookkeeping, non-repair, and non-carrier-shifting.

By Theorem~\ref{thm:endpoint-discriminator-trichotomy}, endpoint-status work on a fixed domain is bookkeeping, governance-equivalent to the already admitted interface, independent same-domain work, or domain-shifting. The listed exclusions remove bookkeeping and domain shift. The single-interface theorem removes governance-equivalence to positive occupation because the act assigns non-occupation of the positive candidate image. The coequal-image route is excluded by Theorem~\ref{thm:negative-coequal-requires-interface-change}. The remaining trichotomy branch is independent same-domain endpoint-status work. On the SAT candidate image, Theorem~\ref{thm:non-positive-status-work-induces-sepdisc} identifies that work with \(\SepDisc(model)\).
\end{proof}
...
On the fixed context-bound SAT endpoint image, ordinary theorem-bearing non-occupation is not a role outside the endpoint-discriminator trichotomy. More precisely, if the accepted CNF-SAT endpoint projection identifies the ordinary negative branch with candidate-image exclusion and that branch is used with official endpoint force, then the induced endpoint role is the separator discriminator:
\[
\begin{gathered}
  \ClayCtx,\quad \FixedDom(R),\quad \ModelOfCnf(model,R),\\
  \OrdThm(\SepBare),\quad \OrdOfficialRes_R(\neg\PosRaw),\\
  \SepBare\leftrightarrow\CandImageExcl(model)
\end{gathered}
\quad\Rightarrow\quad \SepDisc(model).
\]
\end{theorem}

\begin{proof}
Ordinary theoremhood certifies proof legitimacy; it does not determine endpoint-interface role. By Theorem~\ref{thm:ordinary-official-resolution-is-endpoint-use}, ordinary official endpoint resolution is endpoint use rather than support or bookkeeping. By Theorem~\ref{thm:sat-projection-fixes-endpoint-role-content}, and in particular by the accepted carrier projection \(\SepBare\leftrightarrow\CandImageExcl(model)\), the endpoint content of the negative branch on the SAT image is candidate-image non-occupation. Corollary~\ref{cor:candidate-image-exclusion-not-coequal-occupation} and Theorem~\ref{thm:negative-coequal-requires-interface-change} show that this is not positive candidate occupation and not coequal image occupation. The fixed context excludes carrier shift, and official endpoint use excludes support-only and bookkeeping roles. Thus the hypotheses of Theorem~\ref{thm:projected-sat-role-exhaustiveness} are exactly the hypotheses of the alleged middle role. That theorem classifies the act as independent same-domain endpoint-status work, and Theorem~\ref{thm:non-positive-status-work-induces-sepdisc} identifies that work on the SAT image with \(\SepDisc(model)\). Therefore ordinary theorem-bearing non-occupation supplies no fifth endpoint role.
\end{proof}
...
\begin{proof}
The endpoint-use premise activates the kernel and the \(\Aplus\) consequence layer by Theorems~\ref{thm:kernel-necessity} and~\ref{thm:a-plus}. The SAT-local form of the no-independent closure is Theorem~\ref{thm:no-independent-sat-separator-classifier}, which yields \(\NoSepDisc(model)\). Theorem~\ref{thm:no-sepdisc-excludes-sepdisc} then gives \(\neg\SepDisc(model)\). Thus, after projection and trichotomy have identified the separator discriminator, ordinary endpoint use cannot retain that discriminator while exempting it from the same \(\Aplus\) closure.
\end{proof}
...
After the official CNF-SAT resolution-route, carrier-projection, ordinary-resolution, local-countercase, trichotomy, and A+ binding results are in force, the ordinary-practice objection has no remaining middle role or exemption. A critic who accepts the standard CNF-SAT projection, the fixed candidate-image carrier, and the trichotomy classification must now deny one of three named links.
\begin{center}
\small
...
Ordinary theorem-bearing non-occupation & A theorem-bearing, endpoint-resolving, same-domain, non-positive, non-coequal, non-support, non-bookkeeping, non-repair, non-carrier-shifting role that is not independent same-domain endpoint-status work. \\
...
\item Ordinary theorem-bearing non-occupation is not a fifth endpoint role after SAT projection and official endpoint use; proof legitimacy does not neutralize endpoint-role classification.
