...
\[
  \SepBare\Rightarrow\StdLB(model)\Rightarrow\CandImageExcl(model)\Rightarrow\SepImg
\]
...
\begin{theorem}[Candidate endpoint-image exclusion induces theorem-level discrimination]\label{thm:candidate-image-exclusion-theorem-level-discriminator}
...
\begin{proof}
Candidate endpoint-image exclusion globally assigns each encoded polynomial-time candidate the endpoint-relevant status ``non-occupant of the positive CNF-SAT endpoint image.'' This is theorem-level endpoint-status discrimination. It does not compute a failing formula uniformly, does not select a canonical witness, and does not claim an algorithmic separator. Its role is classification of candidate endpoint occupation at theorem level, exactly as formalized by the cited Lean anchor.
\end{proof}
...
In the fixed SAT endpoint setting, the raw negative branch transports through SAT-native lower-bound normal form to the formal SAT image-separator branch:
\[
...
\begin{proof}
The implication is not a reportability principle and does not assert that raw truth creates a Clay report act. By Theorem~\ref{thm:bare-negative-standard-lower-bound-form}, \(\SepBare\) gives standard SAT lower-bound normal form over the fixed encoded candidate image. By Theorem~\ref{thm:standard-lower-bound-candidate-image-exclusion}, that normal form is candidate endpoint-image exclusion. In the formal SAT operator layer, candidate endpoint-image exclusion is exactly the lower-bound/no-positive form of the SAT image-separator branch. The audited anchors \LeanName{cnfSATBareNegativeBranch_standardLowerBoundNormalForm} and \LeanName{cnfSATImageSeparatorBranch_standardLowerBoundNormalForm} record this route. Therefore \(\CnfImageSep(R,model)\), denoted here by \(\SepImg\), follows.

Thus the raw negative SAT branch is not laundered through reportability, not treated as defective theoremhood, and not routed through an additional residual layer. It is transported into the SAT image-separator branch by the SAT-native lower-bound normal form: universal exclusion of encoded polynomial-time candidate occupants from the positive SAT endpoint image.
\end{proof}
...
\begin{proof}
By Theorem~\ref{thm:bare-negative-forces-image}, \(\SepBare\) gives \(\SepImg\). The official-negative-theoremhood lock of Theorem~\ref{thm:official-negative-theoremhood-endpoint-use} blocks the attempted escape in which an ordinary proof of \(\neg\PosRaw\) is offered as the official endpoint resolution while refusing endpoint-use status. Once it is endpoint use, Theorem~\ref{thm:official-negative-use-candidate-image-governance} supplies the final hinge: the endpoint-resolving negative theorem is endpoint-status governance by candidate-image exclusion, not ordinary internal theorem content and not proof-support-only observation. The alleged endpoint-resolving non-governance alternative is the hidden fifth case excluded by Theorem~\ref{thm:endpoint-resolving-nongovernance-hidden-fifth-impossible}.

The occupation-exhaustion and endpoint-status-bivalence locks in Theorems~\ref{thm:sat-negative-occupation-nonoptional}--\ref{thm:negative-governed-endpoint-use-separator} then make explicit that this is not an optional representation choice: a governed official negative endpoint on the fixed CNF-SAT carrier cannot remain a third status. The branch-local bridge theorem gives \(\SepIndBridge(model)\) from the same \(\SepImg\), and Theorem~\ref{thm:image-separator-requires-independent-discriminator} gives \(\SepDisc(model)\). The direct audited formal counterpart is listed in Appendix~A.
\end{proof}
...
\item Candidate-image exclusion can be used as an official endpoint-resolving negative theorem while remaining non-governing, contrary to Theorem~\ref{thm:endpoint-resolving-nongovernance-hidden-fifth-impossible} and Theorem~\ref{thm:official-negative-use-candidate-image-governance}.
...
The proof ladder is ordered so that each theorem cites prior results only. The final endpoint theorem cites the context definition, the standard encoded-model instantiation, the encoded candidate model-adequacy packet, positive direct-occupation theorem, model binding, the branch-local SAT image-separator bridge, raw complementarity, the raw-negative-to-image bridge, image-separator exclusion, and the standard CNF-SAT completeness bridge. No theorem cites itself.
...
\(\CandImageGov\) & endpoint-status governance by candidate-image exclusion & final hinge \\
\begin{tabular}[t]{@{}l@{}}\(\mathsf{CandImage}\)\\\(\mathsf{UseKind}\)\end{tabular} & four-way use classification for candidate-image exclusion & final hinge \\
