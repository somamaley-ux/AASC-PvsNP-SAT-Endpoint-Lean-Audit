...
The solution is AASC-owned. AASC supplies the branch-exclusion proof on the official SAT carrier: kernel necessity, fixed-domain consequence machinery, SAT image-separator asymmetry, and no independent same-domain endpoint-status discriminator. The proof is not a diagonalization argument, circuit lower-bound argument, relativization argument, natural-proofs argument, proof-complexity argument, or constructive algorithm search. Classical complexity theory enters only at the endpoint-correspondence layer: unrestricted finite CNF-SAT is the standard NP-complete carrier, and deterministic polynomial-time decidability of that carrier entails \(\Pclass=\NPclass\) by the Cook--Levin/Karp correspondence. The proof of \(\neg\SepBare\) is AASC-internal.
...
\begin{proof}
By Theorem~\ref{thm:bare-negative-forces-image}, \(\SepBare\) gives \(\SepImg\). The branch-local bridge theorem then gives \(\SepIndBridge(model)\) from that same \(\SepImg\). Theorem~\ref{thm:image-separator-requires-independent-discriminator} gives \(\SepDisc(model)\). The direct audited formal counterpart is listed in Appendix~A.
\end{proof}
...
Objections outside these classes are non-standing unless they identify a theorem-level failure in one of the cited bridges. The relevant question is whether the manuscript reaches the official mathematical endpoint. In particular, it is not a standing objection to say that AASC is an external overlay; in this manuscript AASC machinery is used only as the kernel-forced consequence layer. It is also not a standing objection to say that a negative theorem would be kernel-defective; the manuscript expressly denies that. The eliminated negative route is the raw SAT lower-bound/no-positive branch, because that branch transports directly to the SAT image-separator branch; the image-separator branch then induces independent same-domain endpoint-status discrimination under the SAT-local bridge, and the kernel-forced consequence layer excludes such discrimination.

\section{Conclusion}

The proof has been arranged so that AASC owns the branch-exclusion argument: the kernel is the source of necessity, and AASC is the downstream fixed-domain consequence interface. Raw traces do not become constructions merely by being written. Genuine theoremhood is already governed derivation and is therefore kernel-internal. The positive SAT branch is direct constructive endpoint occupation. The negative SAT branch is not rejected for being negative; what is rejected is the SAT image-separator route, because raw negative branch status transports directly to the image-separator branch, and that branch invokes the branch-local SAT independence bridge and induces an independent same-domain endpoint-status discriminator. The kernel-forced consequence layer excludes such discriminators. Bare complementarity then yields the positive unrestricted finite CNF-SAT endpoint. The Cook--Levin/Karp endpoint correspondence then identifies this AASC-derived endpoint as \(\Pclass=\NPclass\).


\appendix


\section{Lean Formalization Support}
The proof above is the manuscript proof. Lean4 material is appendix-level support for the AASC SAT-operator bridge and negative-branch closure, and reproducibility infrastructure only. The kernel proof and the endpoint-discriminator closure are stated and proved in the main text.

\subsection{Representative audited Lean theorem families}
The available Lean-facing material supports the following families of claims. The names below are audit anchors rather than premises of the manuscript proof.
\begin{enumerate}
\item kernel non-derivability and no-faithful-generator statements;
\item raw-sequence versus governed-construction separation;
\item no same-act repair;
\item AMetric boundary closure;
\item no independent carriers or selectors at the admissibility boundary;
\item uniqueness of the admissible interior;
\item standing conservation;
\item transport and fixed-domain invariance lemmas;
\item CNF-SAT endpoint-context declarations;
\item raw lower-bound/no-positive bridge from the bare SAT negative branch to the SAT image-separator branch;
\item direct positive endpoint occupation;
\item SAT-local image-separator asymmetry;
\item direct raw-negative contradiction under no-independent-discriminator closure;
\item exclusion of the image-separator branch and raw negative branch under no-independent-discriminator closure.
\end{enumerate}

\subsection{SAT operator bridge audit anchors}
The file \Lean{source\_material/SATOperatorProofQueue.lean} contains the branch-asymmetry objects used to audit the manuscript spine. The relevant anchors are:
...
def CnfSATBareNegativeBranch R model : Prop :=\n  Not CnfPositiveEndpoint
...
theorem cnfImageSeparatorBranch_of_not_positiveEndpoint
    (hNotPos : Not CnfPositiveEndpoint) :
    CnfSATImageSeparatorBranch R model
...
\end{lstlisting}
These anchors express the manuscript's branch asymmetry and raw-negative closure: positive endpoint occupation is direct; raw negative branch status transports to the SAT image-separator branch by the lower-bound/no-positive equivalence; the image-separator branch together with the branch-local SAT independence bridge yields an independent endpoint-status discriminator; kernel-forced no-independent-discriminator closure excludes both the image-separator branch and the raw negative branch.

\subsection{Axiom audit note}
Lean support is not used as a substitute for the manuscript proof. The focused SAT operator bridge audit reported only the standard Lean/classical/mathlib base dependencies:
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize]
[propext, Classical.choice, Quot.sound]
\end{lstlisting}
No custom endpoint axiom is required for the branch-asymmetry anchors listed above.
