...
After these exclusions, the proposed middle classification would have to be theorem-bearing, endpoint-resolving, same-domain, non-positive, non-coequal, non-support, non-bookkeeping, non-repair, non-carrier-shifting, and yet not endpoint-status work on the projected SAT image. Theorem~\ref{thm:projected-sat-role-exhaustiveness} rules out that middle role by yielding \(\NonPositiveStatusWork_R(model)\). Theorem~\ref{thm:non-positive-status-work-induces-sepdisc} then identifies that role with \(\SepDisc(model)\).
\end{proof}
